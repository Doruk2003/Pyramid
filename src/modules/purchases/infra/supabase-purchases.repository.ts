...
import { Quote, type QuoteStatus } from '@/modules/purchases/domain/quote.entity';
import { Order, type OrderStatus } from '@/modules/purchases/domain/order.entity';
import type { IPurchasesRepository } from '@/modules/purchases/domain/purchases.repository';
...
export class SupabasePurchasesRepository implements IPurchasesRepository {
    // === QUOTES ===
    async listQuotes(): Promise<Result<Quote[]>> {
        ...
        const quotes = (data as DbQuote[]).map(row => Quote.create({
            id: row.id,
            type: (row.type as 'sale' | 'purchase') || 'purchase',
    ...
    async getNextQuoteNumber(): Promise<string> {
        ...
        if (error || !data) {
            ...
            const { count } = await supabase.from('quotes').select('id', { count: 'exact', head: true }).eq('type', 'purchase');
            return `ATK-${year}-${String((count ?? 0) + 1).padStart(5, '0')}`;
        }
    ...
    async getNextOrderNumber(): Promise<string> {
        ...
        if (error || !data) {
            // Fallback: RPC yoksa count-based
            const year = new Date().getFullYear();
            const { count } = await supabase.from('orders').select('id', { count: 'exact', head: true }).eq('type', 'purchase');
            return `ASP-${year}-${String((count ?? 0) + 1).padStart(5, '0')}`;
        }
        return data as string;
    }
