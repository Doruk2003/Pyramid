...
export class SupabaseInventoryRepository implements IInventoryRepository {
  async getWarehouses(): Promise<Result<Warehouse[]>> {
    ...
    return ok((data as DbWarehouse[] || []).map((row: DbWarehouse) => Warehouse.create({
      id: row.id,
  ...
  async getStockMovements(productId?: string): Promise<Result<StockMovement[]>> {
    ...
    return ok((data as DbStockMovement[] || []).map((row: DbStockMovement) => StockMovement.create({
      id: row.id,
      companyId: row.company_id,
  ...
  async getStockBalances(): Promise<Result<any[]>> {
