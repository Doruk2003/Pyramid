...
async function saveAccount() {
    ...
    if (account.value.iban && !isValidIban(account.value.iban)) {
        toast.add({ severity: 'warn', summary: 'Doğrulama', detail: 'IBAN formatÄ± geçersiz', life: 3000 });
        return;
    }
    if (account.value.taxNumber && !isValidTaxNumber(account.value.taxNumber)) {
        toast.add({ severity: 'warn', summary: 'Doğrulama', detail: 'Vergi No / TC formatÄ± geçersiz', life: 3000 });
        return;
    }
    if (account.value.authorizedGsm && !isValidGsm(account.value.authorizedGsm)) {
        toast.add({ severity: 'warn', summary: 'Doğrulama', detail: 'Yetkili GSM formatÄ± geçersiz', life: 3000 });
        return;
    }
    if (account.value.phone && !isValidGsm(account.value.phone)) {
        toast.add({ severity: 'warn', summary: 'Doğrulama', detail: 'Telefon formatÄ± geçersiz', life: 3000 });
        return;
    }
...
<template>
    <div class="flex flex-col gap-0">
        <div class="card p-6 min-h-32 flex flex-col gap-0">
            <div class="m-0 text-2xl font-medium">Yeni Cari Hesap Oluştur</div>
            <div class="text-surface-600 dark:text-surface-400">
                <p>Cari hesap oluşturma sırasında zorunlu alanları doldurduğunuzdan emin olun.</p>
            </div>
        </div>
